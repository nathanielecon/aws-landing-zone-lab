...
|----|------------|-----------|
| `P_authority` | Authority / harness | Plan, status, harness policies/schemas/tasks, orchestration scripts, integration locks, phase-0 / accuracy evidence manifests, and residual lab-app paths. |
...
**Coverage rule:** every git-tracked file under `continuityops/` except paths under `.terraform/` appears in exactly one partition. Capability-specific docs stay with the owning capability. This partition does not score quality.
