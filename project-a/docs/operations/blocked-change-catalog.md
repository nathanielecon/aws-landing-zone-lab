...
| BC-NET-10 | Supply a typed SG exception CIDR (non-empty `sg_exception_attempts.cidrs`) | Network | Fail-closed `sg_exception_attempts` (default empty/deny; extension-blocked); `rejects_sg_exception_cidr_shape` `expect_failures` | Opens a CIDR-shaped hole in the default-deny workload SG without approved design |
| BC-NET-11 | Supply a typed SG exception port (non-empty `sg_exception_attempts.ports`) | Network | Fail-closed `sg_exception_attempts`; `rejects_sg_exception_port_shape` `expect_failures` | Opens a port-shaped hole in the default-deny workload SG without approved design |
| BC-NET-12 | Supply a typed SG exception protocol (non-empty `sg_exception_attempts.protocol`) | Network | Fail-closed `sg_exception_attempts`; `rejects_sg_exception_protocol_shape` `expect_failures` | Opens a protocol-shaped hole in the default-deny workload SG without approved design |
...
| BC-IAM-01 | Drop required permissions boundary (`require_permissions_boundary = false`) | Identity | Fail-closed `require_permissions_boundary` (must stay true); `rejects_missing_permissions_boundary` `expect_failures` | Removes the hard permission cap on the workload role |
| BC-IAM-02 | Use non–GitHub Actions OIDC trust principal | Identity | `oidc_provider_arn` must match `token.actions.githubusercontent.com`; `rejects_non_github_oidc_trust_principal` | Broadens or mis-scopes workload trust away from GitHub Actions |
| BC-IAM-03 | Override workload actions with over-broad values (for example `*`) | Identity | Fail-closed `workload_action_overrides` (must stay empty); `rejects_overbroad_workload_actions` `expect_failures` | Escalates least-privilege audit writes to unrestricted actions |
| BC-IAM-04 | Omit OIDC trust conditions (`require_oidc_trust_conditions = false`) | Identity | Fail-closed `require_oidc_trust_conditions` (must stay true); `rejects_missing_oidc_trust_conditions` `expect_failures` | Drops aud/sub constraints and widens who can assume the workload role |
...
| BC-AUD-03 | Disable CloudTrail log-file validation (`enable_log_file_validation = false`) | Security Tooling (trail integrity) | Fail-closed `enable_log_file_validation` (must stay true); `rejects_disabled_log_file_validation` `expect_failures` | Removes post-delivery integrity proof for archived trail objects |
| BC-AUD-04 | Disable archive versioning (`enable_archive_versioning = false`) | Log Archive (protected storage) | Fail-closed `enable_archive_versioning` (must stay true); `rejects_disabled_archive_versioning` `expect_failures` | Removes recovery history for overwrite/delete investigation |
| BC-AUD-05 | Enable organization trail (`is_organization_trail = true`) | Security Tooling (org-trail path) | Fail-closed `is_organization_trail` (must stay false); org-trail remains interface-only; `rejects_organization_trail_enabled` `expect_failures` | Org-scoped trail delivery is not enabled in this baseline |
| BC-ORG-01 | Attach SCP to organization root or to an individual account | Management / Organizations | `scp_attachments` accepts only Security, Infrastructure, or Workloads OUs; `tests/governance/organization.tftest.hcl` (`rejects_root_scp_attachment` / `rejects_account_id_scp_attachment`); `tests/governance/Assert-ScpAttachmentNegatives.ps1` (wired via `governance_semantics`) | Root/account attach is outside baseline attachment map |
