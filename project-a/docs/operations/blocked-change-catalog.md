...
| BC-NET-06 | Open public security-group ingress (for example `0.0.0.0/0`) | Network | Default-deny SG (`ingress = []`); `network_negative_tests` assert empty ingress; boundary semantics forbid unrestricted IPv4 ingress | Collapses private workload exposure controls |
...
| BC-AUD-02 | Public ACL / public bucket policy on archive, or missing KMS SSE | Log Archive (protected storage) | `aws_s3_bucket_public_access_block` all true; SSE `aws:kms` via `aws_kms_key.audit`; audit negative tests | Weakens integrity and confidentiality of audit objects |
