...
| BC-NET-06 | Open public security-group ingress (for example `0.0.0.0/0`) | Network | Default-deny SG (`ingress = []`); fail-closed `allow_unrestricted_ingress`; `network_negative_tests` `expect_failures` + empty-ingress assert | Collapses private workload exposure controls |
