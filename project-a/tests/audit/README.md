...
(`enable_log_file_validation = false`), and disabled archive versioning
(`enable_archive_versioning = false`). A separate happy-path run asserts Log
Archive public-access block and KMS SSE when inputs are valid. It does not call
live log services or deploy the audit path.
