...
- Break (claim): Cloud seat preflight fails `NoCredentials` while
  `CURSOR_AWS_ASSUME_IAM_ROLE_ARN` points at `<AWS_ACCOUNT_ID>` /
...
- Break: Renamed `nathanielecon/cloud` → `nathanielecon/aws-landing-zone-lab`.
  Every Landing Zone workflow failed at **Configure AWS credentials (OIDC)**
  with `Not authorized to perform sts:AssumeRoleWithWebIdentity` on
  `project-a-lzlab-gha`. Last healthy apply: run `29430449964` (~15:58Z) while
  the repo was still `cloud`. First post-rename failure: `29436984325`.
  Self-heal in `plan-lab.sh` / `apply-lab.sh` could not run (needs the assume
  that was denied). Cloud Agent: `NoCredentials` (expected).
- Why rename-back also failed: GitHub renames on/after **2026-07-15** switch
  OIDC to **immutable subject claims**
  (`repo:OWNER@OWNER_ID/REPO@REPO_ID:...`). Temporary rename-back to `cloud`
  still failed (plan `29441863919`, apply `29441978207`) — not a one-name
  mismatch. Expected PR sub after rename:
  `repo:nathanielecon@177059064/aws-landing-zone-lab@1296742987:pull_request`.
- Wrong fix attempt: CloudShell paste with name-only
  `repo:nathanielecon/*:...` trust applied cleanly but plan `29452275836`
  still denied OIDC.
- Fix (operator CloudShell, account `<AWS_ACCOUNT_ID>`): update trust to immutable
  `sub` + `repository_id` / `repository_owner_id` lock:

  ```bash
  curl -fsSL https://paste.rs/4BEzo | bash
  # or: bash platform/sandbox/landing-zone-lab/ci-bootstrap/fix-oidc-trust-cloudshell.sh
  ```

  Live trust `StringLike` subs:
  `repo:nathanielecon@177059064/*@1296742987:(main|pull_request|cursor/*|environment:lab)`
  with `repository_id=1296742987`, `repository_owner_id=177059064`.
- Repo: same shape in `ci-bootstrap/` + `plan-lab.sh` / `apply-lab.sh` retarget;
  merged via PRs `#25` / `#26` (`f688065`, `4065330`).
- Verify: plan OIDC green
  [29452598739](https://github.com/nathanielecon/aws-landing-zone-lab/actions/runs/29452598739)
  (`assumed-role/project-a-lzlab-gha/gha-lzlab-plan-29452598739`); apply on
  `main` green
  [29452719184](https://github.com/nathanielecon/aws-landing-zone-lab/actions/runs/29452719184)
  (`gha-lzlab-apply-29452719184`); plan/apply retarget re-synced live trust.
...
- Break: Two live CloudTrail trails were billing in parallel in account `<AWS_ACCOUNT_ID>` — `project-a-lzlab-trail` and `project-a-sandbox-trail`, both multi-region with `IncludeGlobalServiceEvents: true`. Found on 2026-08-08 while investigating an unrelated ~$300 cost overrun in `nathanielecon/ContinuityOps` (its BF-2026-029..032), not by any control in this repo. AWS bills only the *first* copy of management events per account, so the second trail's management events are chargeable, and both archive buckets accrue S3 storage indefinitely. Measured in Cost Explorer over full days 2026-08-06/07: CloudTrail **$0.222/day** (~$6.75/mo) and KMS **$0.0968/day** for three customer-managed keys (~$2.90/mo, matching 3 × $1 to the cent).
  Root cause: `platform/sandbox/aws-proof` and `platform/sandbox/landing-zone-lab/lab` each instantiate the shared `platform/terraform/audit` module with a different `name_prefix`, so each creates its own trail, archive bucket, KMS key and Config recorder. That duplication is correct by design — the sandbox root exists to prove the module applies live — but nothing tore the sandbox down after it had served that purpose, and it has been billing since 2026-07-13.
  Fix (proposed, not yet applied): `terraform destroy` the `platform/sandbox/aws-proof` root — 13 resources per its `EVIDENCE.md`. Safe because that root's claim is already fully captured in `platform/sandbox/aws-proof/EVIDENCE.md`, including the trail name, archive bucket, `alias/project-a-sandbox-audit` → `cd27223d-5d4a-432e-8f71-7eb2b3462781` (verified still live and matching on 2026-08-08), and the Config recorder. `evidence/README.md` already designates `landing-zone-lab` as the live lab, so the governance claim rests there and survives untouched.
  Caution on KMS: scheduled key deletion has a mandatory 7–30 day waiting period, and `alias/project-a-lzlab-tfstate` (a *different* key, from `ci-bootstrap`) encrypts Terraform state — destroying that one would make the state permanently unreadable. Only the sandbox key is in scope.
  Residual: the identity used for both sandbox applies was an **account root session** (recorded in `aws-proof/EVIDENCE.md` as "prefer an IAM user/role for future ops"). The same root-credential pattern was independently found in Project C's `governing-manifest.json` and in an MCP connector during the ContinuityOps investigation — three places, so it is a habit rather than a one-off. A scoped read-only IAM user `continuityops-agent-ro` now exists in the account as a replacement for cost/inventory work.
