...
try {
    ...
    foreach ($validator in $policy.validators) {
        ...
        switch ($id) {
            ...
            'diagram_links' {
                Assert-ExpectedArtifacts;$required=@('aws-landing-zone-lab.png','aws-landing-zone-architecture.png','aws-landing-zone-how-it-works.png','aws-landing-zone-network.png');$docs=Get-ProjectText @('project-a/README.md','project-a/docs','README.md') @('.md');$errors=@()
                foreach($name in $required){$path=Join-Path $Root "project-a/docs/diagrams/$name";if(-not(Test-Path $path -PathType Leaf)){$errors+="missing $name";continue};$bytes=[IO.File]::ReadAllBytes($path);if($bytes.Length -lt 8 -or $bytes[0] -ne 0x89 -or $bytes[1] -ne 0x50 -or $bytes[2] -ne 0x4E -or $bytes[3] -ne 0x47){$errors+="not PNG $name"};if($docs -notmatch [regex]::Escape("diagrams/$name")){$errors+="unlinked $name"}}
                $message=if($errors){$errors -join ', '}else{'Required Image2 PNG diagrams exist and are linked from documentation.'};Add-Result $id ($errors.Count -eq 0) $message $timer.ElapsedMilliseconds
            }
            'graphify_evidence' {
