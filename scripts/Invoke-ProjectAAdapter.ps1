...
function Invoke-ModelProcess([string[]]$Arguments, [string]$Label, [string]$StandardInput = '') {
    ...
    try {
        ...
        $stdout = Protect-LogText -Text $stdoutTask.GetAwaiter().GetResult(); $stderr = Protect-LogText -Text $stderrTask.GetAwaiter().GetResult(); $code = $process.ExitCode
...
function Invoke-Gate([switch]$Committed) {
    ...
    try{[void]$process.Start();$stdoutTask=$process.StandardOutput.ReadToEndAsync();$stderrTask=$process.StandardError.ReadToEndAsync();$process.WaitForExit();$output=$stdoutTask.GetAwaiter().GetResult();$stderr=$stderrTask.GetAwaiter().GetResult();$code=$process.ExitCode}finally{$process.Dispose()}
...
function Write-ApprovalRequest($GateResult, $Diff) {
    ...
    $state.status='awaiting_approval'; $state.validation_digest=[string]$GateResult.validation_digest; $state.diff_sha256=$Diff.sha256; $state.approval_request=$requestPath
    Save-State
...
function Resume-PendingCommit {
    ...
    $indexed=@(& git -C $root -c core.quotepath=false diff --cached --name-only|ForEach-Object{$_-replace'\\','/'}|Sort-Object)
    ...
    foreach($path in @($requestPath,$receiptPath,$keyPath)){Remove-Item -LiteralPath $path -Force -ErrorAction SilentlyContinue}
}
...
function Finish-Or-Pause($GateResult) {
    ...
    if ([bool]$policy.approval.required) {
        ...
        $verifyOutput = & (Join-Path $PSScriptRoot 'Test-ProjectAApproval.ps1') -Root $root -PolicyPath $policyPath -RequestPath $requestPath -ReceiptPath $receiptPath -KeyPath $keyPath
...
if (Test-Path -LiteralPath $statePath) {
    ...
    if ([string]$state.starting_commit -ne $head) {
        ...
        $tree=(& git -C $root rev-parse "$head^{tree}").Trim();$commitPaths=@(& git -C $root -c core.quotepath=false diff-tree --no-commit-id --name-only -r $head|Sort-Object)
...
$existing = @(Get-ChangedPaths -Root $root)
...
while ($true) {
    $before = Get-DiffFingerprint -Root $root
    if ($state.phase -eq 'terra') {
        ...
        $after=Get-DiffFingerprint -Root $root; $noDiff=$before -eq $after; $state.consecutive_failures=[int]$state.consecutive_failures+1
