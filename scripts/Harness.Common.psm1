...
function Assert-StrictJsonContractForPath {
    ...
    if ($normalized -like '*/platform/harness/bundle-approval.json') {
    ...
    if ($normalized -like '*/platform/harness/execution-approval.json') {
    ...
    if ($normalized -like '*/.harness/runtime/platform/PRD.json') {
    ...
    if ($normalized -like '*/.harness/runtime/platform/state/*.json') {
    ...
    if ($normalized -like '*/evidence/platform/*.json') {
    ...
    if ($normalized -like '*/RalphyHarness/cloud/approvals/platform/requests/*.json') {
    ...
    if ($normalized -like '*/RalphyHarness/cloud/approvals/platform/receipts/*.json') {
...
function Get-HarnessLifecycleExcludedPaths {
    ...
    if ($ProfileId -eq 'smoke') {
        ...
        $paths.Add('.harness/runtime/platform/PRD.json')
        foreach ($taskId in @($profile.task_ids | ForEach-Object { [string]$_ })) {
            $paths.Add(".harness/runtime/platform/state/$taskId.json")
            $paths.Add(".harness/runtime/platform/locks/$taskId.lock")
            $paths.Add(".harness/runtime/platform/takeovers/$taskId.md")
        }
    }
...
function Test-ProjectACompletedTaskState {
    ...
    $policyPath = Join-Path $Root "platform/harness/tasks/$TaskId.json"
...
function Get-ReconstructedProjectACompletedState {
    ...
    $commit = $null
    $evidenceIndexPath = Join-Path $Root 'platform/evidence-index.md'
...
function Sync-ProjectAManifestWithTaskState {
    ...
    foreach ($task in $manifest.tasks) {
        $id = Get-TaskIdFromArguments -Arguments @([string]$task.title)
        $statePath = Join-Path $Root ".harness/runtime/platform/state/$id.json"
