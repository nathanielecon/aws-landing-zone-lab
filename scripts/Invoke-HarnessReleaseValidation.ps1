...
$pinsPath = Join-Path $Root 'harness/tool-versions.json'
$expected = [ordered]@{
    terraform = '1.15.5'
    ralphy    = '4.7.2'
    node      = '24'
...
if (Test-Path -LiteralPath $pinsPath -PathType Leaf) {
    ...
    Write-Host "Loaded tool pins from harness/tool-versions.json (terraform=$($expected.terraform), ralphy=$($expected.ralphy); Node expected major $($expected.node))."
} else {
    Write-Host "harness/tool-versions.json missing; expecting Terraform $($expected.terraform), ralphy $($expected.ralphy), Node $($expected.node)."
}
...
Write-Host 'Skipping Project A bundle verify (harness-only repo).'
