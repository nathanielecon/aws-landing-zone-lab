...
```powershell
pwsh -NoLogo -NoProfile -File platform/tests/governance/Assert-ScpAttachmentNegatives.ps1
```
...
```powershell
terraform -chdir=platform/terraform/organization test -no-color -test-directory=.harness-tests
```
