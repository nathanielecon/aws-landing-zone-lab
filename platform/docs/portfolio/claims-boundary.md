...
`platform/sandbox/landing-zone-lab` was **cloud-validated** in AWS account
`<AWS_ACCOUNT_ID>` / `us-east-1` via **GitHub OIDC → Terraform CI** (role
`project-a-lzlab-gha`, workflow `.github/workflows/landing-zone-lab.yml`).
Live resources: identity (OIDC/workload role), private VPC + flow logs, and
audit (CloudTrail/Config/KMS Log Archive). See
`platform/sandbox/landing-zone-lab/EVIDENCE.md` and Actions run
`29366105164`. Cursor Cloud Agents are **not** the AWS apply control plane.
...
`platform/sandbox/landing-zone-lab/ORGS_INTERFACE.md`.

The earlier audit-only sandbox (`platform/sandbox/aws-proof`) remains historical
...
handoff language, and an honest single-account live lab for
identity/network/audit — superior to audit-only sandbox proof, inferior to
real multi-account cloud validation.
...
the lab evidence files for the specific live resources shown there. Do not
claim Cursor Cloud Agent AWS apply for this lab.
