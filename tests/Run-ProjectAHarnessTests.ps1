...
'@
    ...
    $codexSh = Join-Path $path 'tests/fixtures/codex'
    $codexBody = @'
#!/bin/sh
ROOT="$(CDPATH= cd -- "$(dirname "$0")" && pwd)"
exec pwsh -NoLogo -NoProfile -File "$ROOT/Fake-Codex.ps1" "$@"
'@
    [IO.File]::WriteAllText($codexSh, ($codexBody -replace "`r`n", "`n"), [Text.UTF8Encoding]::new($false))
...
"@,[Text.UTF8Encoding]::new($false))
    ...
    $ralphySh = Join-Path $toolRoot 'ralphy'
    $ralphyBody = @'
#!/bin/sh
ROOT="$(CDPATH= cd -- "$(dirname "$0")" && pwd)"
exec pwsh -NoLogo -NoProfile -File "$ROOT/Fake-Ralphy.ps1" "$@"
'@
    [IO.File]::WriteAllText($ralphySh, ($ralphyBody -replace "`r`n", "`n"), [Text.UTF8Encoding]::new($false))
    if (-not ($IsWindows -or $env:OS -match 'Windows')) { & chmod +x -- $ralphySh }
    # Contract fixtures must not depend on a host Terraform install. Start-ProjectAHarness
    # accepts terraform.cmd when terraform.exe is absent (and plain `terraform` on Unix);
    # plant pinned version stubs first on PATH.
    ...
    $tfSh = Join-Path $toolRoot 'terraform'
    $tfBody = @'
#!/bin/sh
if [ "$1" = version ] && [ "$2" = -json ]; then printf '%s\n' '{"terraform_version":"1.15.5","platform":"linux_amd64","provider_selections":{},"terraform_outdated":false}'; exit 0; fi
echo Unsupported fake terraform invocation >&2; exit 1
'@
    [IO.File]::WriteAllText($tfSh, ($tfBody -replace "`r`n", "`n"), [Text.UTF8Encoding]::new($false))
    if (-not ($IsWindows -or $env:OS -match 'Windows')) { & chmod +x -- $tfSh }
    $saved=@{
        PATH=$env:PATH
    ...
    try{
        ...
        $env:PATH = ($toolRoot + $pathSep + (Join-Path $Workspace 'tests/fixtures') + $pathSep + $env:PATH)
...
Assert-True (Test-AllowedPath -Path 'project-a/docs/diagrams/aws-landing-zone-network.png' -AllowedPaths @('project-a/docs/diagrams/aws-landing-zone-network.png')) 'allowlist accepts the exact A-004 network diagram path'
Assert-True (-not (Test-AllowedPath -Path 'project-a/docs/diagrams/network.svg' -AllowedPaths @('project-a/docs/diagrams/aws-landing-zone-network.png'))) 'allowlist still rejects unrelated A-004 diagram siblings'
...
try{$exclusiveLockPath=Join-Path $exclusiveLockRepo '.harness/runtime/project-a/locks/A-006.lock';$heldExclusive=Open-ExclusiveLock -Path $exclusiveLockPath;try{Assert-ThrowsLike { Open-ExclusiveLock -Path $exclusiveLockPath | Out-Null } 'Another harness instance owns the lock' 'lock ownership: Open-ExclusiveLock FileShare.None rejects a second open'}finally{$heldExclusive.Dispose()}}finally{Remove-Item -LiteralPath $exclusiveLockRepo -Recurse -Force;Remove-Item -LiteralPath $exclusiveLockLocal -Recurse -Force -ErrorAction SilentlyContinue}
...
try{Write-TestPolicy $digestRecoveryRepo 'A-001' $true;& git -C $digestRecoveryRepo add project-a/harness/tasks/A-001.json;& git -C $digestRecoveryRepo commit -m policy|Out-Null;Set-AdapterEnvironment $digestRecoveryRepo 'A-001' $digestRecoveryLocal;$code=Invoke-ProjectAAdapterFixture -Arguments @('exec','--json','[TASK:A-001]');Assert-True ($code -eq 75) 'digest recovery fixture reaches approval pause';Write-ApprovalConfirmationFixture -LocalData $digestRecoveryLocal -TaskId 'A-001';& pwsh -NoLogo -NoProfile -NonInteractive -File (Join-Path $root 'scripts/Approve-ProjectATask.ps1') -TaskId A-001 -Root $digestRecoveryRepo|Out-Null;Assert-True ($LASTEXITCODE -eq 0) 'digest recovery fixture receives an approval';$env:HARNESS_TEST_KILL_POINT='after_commit';$code=Invoke-ProjectAAdapterFixture -Arguments @('exec','--json','[TASK:A-001]');Assert-True ($code -eq 91) 'approval recovery interruption occurs after commit';Remove-Item Env:HARNESS_TEST_KILL_POINT;$statePath=Join-Path $digestRecoveryRepo '.harness/runtime/project-a/state/A-001.json';$tamperedState=Read-JsonFile -Path $statePath;$tamperedState.approval_receipt_digest=('0'*64);Write-JsonNoBom -Path $statePath -Value $tamperedState;$code=Invoke-ProjectAAdapterFixture -Arguments @('exec','--json','[TASK:A-001]');$recoveredState=Read-JsonFile -Path $statePath;Assert-True ($code -ne 0 -and $recoveredState.status -ne 'completed' -and @(Get-Content -LiteralPath (Join-Path $digestRecoveryRepo '.logs/calls.txt')).Count -eq 1) 'tampered approval digest rejects interrupted-commit recovery without another model call'}finally{Clear-AdapterEnvironment;Remove-Item -LiteralPath $digestRecoveryRepo -Recurse -Force;Remove-Item -LiteralPath $digestRecoveryLocal -Recurse -Force -ErrorAction SilentlyContinue}
    ...
    Remove-Item -LiteralPath $digestRecoveryRepo -Recurse -Force -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $digestRecoveryLocal -Recurse -Force -ErrorAction SilentlyContinue
    Write-Host 'SKIP: DPAPI digest-recovery asserts require Windows ProtectedData'
}
...
# Resume boundary: Get-CanonicalDiffRecord dirty-set mismatch (same helper Start-ProjectAHarness uses).
$resumeDirtyRepo=New-TestRepo 'resume dirty-set mismatch'
try{
    [IO.Directory]::CreateDirectory((Join-Path $resumeDirtyRepo 'project-a'))|Out-Null
    $resumePath=Join-Path $resumeDirtyRepo 'project-a/fixture with space-非.txt'
    [IO.File]::WriteAllText($resumePath,"resume-baseline`n",[Text.UTF8Encoding]::new($false))
    $persistedDiff=(Get-CanonicalDiffRecord -Root $resumeDirtyRepo -AllowedPaths @('project-a/**')).sha256
    [IO.File]::AppendAllText($resumePath,'x')
    $mutatedDiff=(Get-CanonicalDiffRecord -Root $resumeDirtyRepo -AllowedPaths @('project-a/**')).sha256
    Assert-True ($mutatedDiff -ne $persistedDiff) 'resume dirty-set mismatch: allowed-path mutation invalidates persisted diff identity'
}finally{Remove-Item -LiteralPath $resumeDirtyRepo -Recurse -Force}
