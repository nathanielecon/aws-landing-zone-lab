...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $root 'platform/harness/bundle-approval.json') -Destination $copiedBundle
    Copy-Item -LiteralPath (Join-Path $root 'platform/harness/execution-approval.json') -Destination $copiedExec
    ...
    Copy-Item -LiteralPath (Join-Path $root 'platform/harness/bundle-approval.json') -Destination $copiedBundle -Force
    Copy-Item -LiteralPath (Join-Path $root 'platform/harness/execution-approval.json') -Destination $copiedExec -Force
    ...
    $schemaPath = Join-Path $root 'platform/harness/policy.schema.json'
    $policySrc = Join-Path $root 'platform/harness/tasks/A-006.json'
    ...
    $approvalRequiredSrc = Join-Path $root 'platform/harness/tasks/A-001.json'
